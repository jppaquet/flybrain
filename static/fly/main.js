/* main.js - scène three.js, boucle d'animation et panneau de contrôle de la mouche. */
...
// piste : sol brillant + dalles lumineuses additives
...
const fly = createFly();
scene.add(fly.root);
const grid = new THREE.GridHelper(80, 160, 0x2c2c31, 0x1d1d21);   // repère au sol pour voir la marche
...
function setCam(name) {
  const o = new THREE.Vector3(controls.target.x, 0, controls.target.z + 0.1);   // relatif à la mouche
...
const JOINTS = [
  ["height", "Hauteur", 0.35, 0.8, v => `${v.toFixed(2)} mm`],
  ["pitch", "Tangage", -0.5, 0.5, deg], ["roll", "Roulis", -0.4, 0.4, deg],
  ["yaw", "Orientation", -Math.PI, Math.PI, deg], ["stance", "Écartement pattes", 0.75, 1.3, v => `${v.toFixed(2)}×`],
  ["headYaw", "Tête : lacet", -0.7, 0.7, deg], ["headPitch", "Tête : tangage", -0.6, 0.6, deg],
  ["abdPitch", "Abdomen : lever", -0.5, 0.8, deg], ["abdYaw", "Abdomen : côté", -0.5, 0.5, deg],
  ["wingSpreadL", "Aile gauche", 0, 1, pct], ["wingSpreadR", "Aile droite", 0, 1, pct],
  ["proboscis", "Proboscis", 0, 1, pct], ["antenna", "Antennes", -0.4, 0.6, deg],
];
let base = { ...REST };
const sliders = {};
...
audio.el = $("#player");
let volume = 0.8;
const LABEL = { demo: "beat de démo · 98 BPM", mic: "micro", none: "aucune source" };
function setSource(mode, name) {
  ...
  status(mode === "none" ? "Source coupée" : `Source : ${mode === "file" ? name : LABEL[mode]}`);
}
...
async function playFile(file) {
  ...
  catch (err) { status(`Lecture impossible : ${err.message}`, true); }
}
$("#src-file").addEventListener("change", e => { const f = e.target.files[0]; if (f) playFile(f); e.target.value = ""; });
$("#player").addEventListener("error", () => status("Format audio non pris en charge par le navigateur", true));
$("#src-mic").addEventListener("click", async () => {
  try { await audio.useMic(); setSource("mic"); }
  catch (err) { status(`Micro indisponible : ${err.message}`, true); }
});
...
function drawSig(t) {
  ...
  g.fillText("seuil", w - 2, y(audio.threshold) - 3);
  g.textBaseline = "top"; g.fillText("maintenant", w, bot + 3); g.textAlign = "left"; g.fillText("−6 s", 0, bot + 3);
...
function frame(ts) {
  ...
  if (mode === "brain") {
    ...
    followCamera();
    if ($("#live-audio").checked && brain.id != null && now - lastAudio > 0.1) {
      lastAudio = now; brain.audio(f.active ? f.level : 0);
    }
  } else {
  ...
  if (now - lastText > 0.12) {
    ...
    const state = mode === "brain" ? brainChip()
      : f.mode === "none" ? ["", "Aucune source audio"]
      : !f.active ? ["listening", "À l'arrêt · silence"]
      : !opts.enabled ? ["listening", "Son détecté · danse désactivée"]
      : ["dancing", `En train de twerker${f.bpm ? ` · ${Math.round(f.bpm)} BPM` : ""}`];
    if (state[1] !== lastChip) {
      lastChip = state[1];
...
/* ----------------------------------------------------------- mode cerveau */
const GROUPS = ["locomotion", "saut", "proboscis", "ailes", "tête", "abdomen"];
const GROUP_LABEL = { locomotion: "Neurones descendants · locomotion", saut: "Saut", proboscis: "Proboscis",
                      ailes: "Ailes et haltères", "tête": "Tête", abdomen: "Abdomen" };
const FUNCS = [["pro", "pro"], ["rem", "rem"], ["trx", "tr ext"], ["trf", "tr flx"],
               ["tix", "ti ext"], ["tif", "ti flx"], ["tad", "ta dép"], ["tal", "ta rel"]];
const side = s => s.replace(/ L$/, " G").replace(/ R$/, " D");
...
function setMode(m) {
  ...
  $("#mode-note").textContent = m === "brain"
    ? "Le corps suit les neurones descendants et les motoneurones de la simulation du connectome."
    : "Chorégraphie calée sur le son : aucun neurone n'est simulé.";
  if (m === "dance") { brain.stop(); $("#live-start").textContent = "Démarrer la simulation"; body.reset(); fly.root.position.set(0, 0, 0); followCamera(); }
  lastChip = "";
}
...
function buildBrainUI(info) {
  ...
  for (const kind of ["sens", "opto"]) {
    $(kind === "sens" ? "#ev-sens" : "#ev-opto").replaceChildren(...info.events.filter(e => e.kind === kind).map(e => {
      ...
      b.title = `${e.n} neurones · ${e.query}${e.side ? ` · côté ${e.side === "L" ? "gauche" : "droit"}` : ""} · ${e.hz} Hz pendant ${e.ms} ms`;
      b.addEventListener("click", () => { brain.event(e.key); body.note(`Stimulus : ${e.label} (${e.n} neurones)`); });
      return b;
    }));
  }
  const bars = $("#chan-bars"); bars.replaceChildren(); chanRows = {};
  const rows = info.channels.filter(c => c.group !== "pattes").sort((a, b) => GROUPS.indexOf(a.group) - GROUPS.indexOf(b.group));
  let g = null;
  for (const c of rows) {
    if (c.group !== g) { g = c.group; bars.append(el("div", "g", GROUP_LABEL[g] || g)); }
    const lab = el("span", null, side(c.label)); lab.title = `${c.n} neurone${c.n > 1 ? "s" : ""}`;
  ...
  for (const leg of ["T1", "T2", "T3"]) for (const s of ["L", "R"]) {
    grid.append(el("span", "r", `${leg} ${s === "L" ? "G" : "D"}`));
    for (const [f, l] of FUNCS) {
      const key = `${leg}${s}_${f}`, c = chanInfo[key], cell = el("div", c ? "c" : "c na");
      cell.title = c ? `${side(c.label)} · ${c.n} MN` : `${leg} ${l} : aucun motoneurone annoté`;
...
function updateBrainUI() {
  ...
  for (const [k, c] of Object.entries(legCells)) {
    ...
    c.title = `${side(chanInfo[k].label)} · ${chanInfo[k].n} MN · ${Math.round(S[k] || 0)} Hz`;
  }
  const st = brain.status;
  $("#live-state").textContent = brain.id == null ? "arrêté" : st ? `t ${(st.t_ms / 1000).toFixed(1).replace(".", ",")} s` : "démarrage…";
  if (brain.error) status(`Simulation : ${brain.error}`, true);
  const log = body.events.slice(-4);
  $("#evlog").replaceChildren(...(log.length ? log : [[body.t, "Démarre la simulation puis envoie un stimulus"]]).map(([t, txt]) => {
    const d = el("div"); d.append(el("b", null, `${t.toFixed(1).replace(".", ",")} s`), txt); return d;
  }));
}

function brainChip() {
  if (brain.id == null) return ["", "Cerveau · simulation arrêtée"];
  const st = brain.status;
  if (!st) return ["listening", "Cerveau · démarrage…"];
  const n = brain.cur ? brain.cur[3] : 0, spd = String(st.speed.toFixed(2)).replace(".", ",");
  return [n > 0 ? "dancing" : "listening", `Cerveau · ${spd}× temps réel · ${n.toLocaleString("fr-FR")} neurones actifs`];
}
...
async function startLive(note) {
  try {
    ...
    buildBrainUI(info);
    $("#live-start").textContent = "Arrêter";
    status(`Simulation en continu · ${info.channels.length} canaux moteurs lus`);
    if (note) body.note(note);
  } catch (e) { status(`Simulation impossible : ${e.message}`, true); }
}
$("#live-start").addEventListener("click", () => {
  if (brain.id != null) { brain.stop(); $("#live-start").textContent = "Démarrer la simulation"; return; }
  startLive();
});
$("#live-zero").addEventListener("click", () => startLive("Réseau remis au repos"));
$("#live-reset").addEventListener("click", () => { body.reset(); });

requestAnimationFrame(frame);
status("Prêt · choisis une source sonore : beat de démo, fichier ou micro");

window.fly = {
  get pose() { return { ...base }; },
  set(k, v) { if (k in base) { base[k] = +v; syncSliders(); } },
  audio, dancer, model: fly, scene, camera,
};
