/* dance.js - behaviours layered on top of the manual pose: rest (breathing,
   antennae) and a twerk locked to the phase of the musical beat. */

const frac = x => x - Math.floor(x);
// "pop": fast rise on the beat, exponential fall
...
export class Dancer {
  ...
  /* f: audio features; o: {enabled, intensity, cadence, song}; base: manual pose */
  update(dt, f, o, base) {
    this.t += dt;
    ...
    const sway = Math.sin(Math.PI * ph);                 // sway over 2 beats
    const p = { ...base };
    ...
    // twerk: leaning forward, legs bent, the abdomen snaps on the beat
    p.pitch += E * 0.22;
    ...
    p.roll += E * 0.07 * sway;
    p.headPitch -= (p.pitch - base.pitch) * 0.8 - E * 0.05 * p1;   // gaze stabilisation + nod
    p.antenna += E * 0.18 * p1;
    const open = E * 0.28;                                       // wings spread to free the abdomen
    p.wingSpreadL = Math.max(p.wingSpreadL, open);
    p.wingSpreadR = Math.max(p.wingSpreadR, open);

    // courtship song: on the downbeat, one wing extends and vibrates (alternating sides)
