#!/usr/bin/env python3
"""
MaleCNS v1.0 -> compact graph for spiking simulation.

Reads the .feather files of the flat connectome (Janelia/Google Research, CC-BY) and writes:
  - malecns_graph.npz : CSR (indptr, indices, weight) of the neuron->neuron graph
  - neurons.csv       : idx, body, type, class, side, nt, sign  (one row per neuron)

Usage: python convert.py [directory]
"""
import sys, os, csv
import numpy as np
...
MIN_WEIGHT = int(os.environ.get("MIN_WEIGHT", "3"))   # minimum synapse count to keep a connection
...
# keep the bodies identified as neurons (type or class filled in)
keep = np.array([bool(t) or bool(k) for t, k in zip(types, klass)])
print(f"  {keep.sum():,} bodies kept as neurons out of {len(bodies):,}")
...
INHIB = ("gaba", "glut")            # GABA and glutamate = inhibitory in Drosophila
sign = np.ones(N, dtype=np.int8)
for i, v in enumerate(nt):
    if any(v.startswith(x) for x in INHIB): sign[i] = -1
print(f"  {(sign < 0).sum():,} inhibitory neurons / {N:,}")

# ------------------------------------------------------------------- connections
# read batch by batch, filtered on the fly: ~1 GB of RAM instead of ~10 GB
print("Reading the connection graph (large file, read in batches)…")
with pa.memory_map(W_FILE) as src:
    rd = ipc.open_file(src)
    wcols = rd.schema.names
    print(f"\n[connectome-weights] {rd.num_record_batches:,} batches")
    for f in rd.schema:
        print(f"    {f.name:<28} {f.type}")
...
del parts
print(f"  {n_raw:,} raw pairs")
print(f"  {len(pre):,} pairs with weight >= {MIN_WEIGHT}")
...
del pre, post
print(f"  {len(ip):,} connections between identified neurons")
...
print(f"\nOK -> {out}  ({os.path.getsize(out)/1e6:.0f} MB)")
...
print(f"OK -> neurons.csv  ({N:,} neurons)")
